...
- 스펙 기준: [SSOT.md](./SSOT.md) (시나리오 6개 × 엔드포인트 13개), 용어: [GLOSSARY.md](./GLOSSARY.md), 디자인: [DESIGN.md](./DESIGN.md)
...
| 라우트 | Flow | 설명 |
|---|---|---|
| `#/connect` | 1 | Base URL(US/JA/커스텀) + API 키 검증(`GET /v2/projects`), OWN-USER-ID 설정 |
...
- 생성된 코드에는 키가 절대 삽입되지 않는다 — curl/Node/Python은 환경변수 `ALLI_API_KEY`, 브라우저 변형은 placeholder + 경고 주석(운영에서는 서버 경유 필수).
...
문서화되지 않은 응답 스키마(SSOT §9)는 목 픽스처에 **ASSUMPTION** 주석으로 가정돼 있다.
실 자격증명 확보 후 아래를 수행해 가정을 실측으로 교체할 것:

1. **CORS 실측** — `#/connect`에서 실 키로 프로젝트 확인.
2. **run 응답 형태(§9-3)** — `#/run`에서 답변형 앱 실행 → Raw 뷰 "응답 복사" → `result.responses[]` vs `result.choices[]` 확정.
3. **run_conversation 스트림(§9-2)** — `#/multipart`에서 실행 → Raw 뷰의 청크 트랜스크립트 복사 → conversationId 위치 확정.
4. **GA 스트림 프레이밍** — `#/answer` stream 모드 → 청크 트랜스크립트 캡처.
5. 캡처로 `src/mock/fixtures.ts`(ASSUMPTION 교체), 필요시 `src/core/extract.ts`(우선 경로 고정), `SSOT.md` §9 갱신.
...
| 변수 없이 실행 | `{"errors":"internal error. Expecting value…"}` (§9-1 불친절 에러) |
| 앱 "레거시 요약" 실행 | `result.choices[]` 레거시 응답 형태 (§9-3) |
| 업로드 파일명에 `fail` 포함 | 인제스천 `parsing_fail` → 롤백 경로 (Flow 5) |
| OWN-USER-ID 미설정 + 답변 생성 | threadId 미반환 (멀티턴 비활성, §9-6) |
