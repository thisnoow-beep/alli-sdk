...
> ✅ **Gate G1 실측 (2026-06-16)** — 아래 7000~7004 코드표는 **실 호출에서 관측되지 않았다**. 실제 형태:
>
> | 유발 | HTTP | 실제 본문 |
> |---|---|---|
> | 잘못된/누락 키 | **401** | `{"error":{"value":1013,"name":"INVALID_TOKEN"},"message":"INVALID_TOKEN"}` |
> | 깨진 JSON 본문 | **400** | `{"result":[],"errors":"content must be JSON deserializable!"}` |
> | 잘못된 메서드 | **405** | `Method not allowed` (text/html, **비JSON**) |
> | inputs 형식 오류(일부 앱) | 500 | `{"errors":"internal error. Expecting value: ..."}` |
>
> 인증 에러 코드는 `error.value`(숫자 1013)·`error.name`('INVALID_TOKEN')에 **중첩**되고 HTTP는 401(403 아님).
> `errors.ts`는 `{error:{value,name}}`(error-object)·`error`(문자열)·`errors`·비JSON을 모두 처리하며,
> 키 안내는 `error.name==='INVALID_TOKEN'` 또는 401에서 발화한다.

레거시/참고용 코드표 (미확인 — 실 호출에서 관측되지 않음, back-compat용으로만 파서가 보존):
...
- `mode: sync`(기본) | `stream`. ✅ **Gate G1 실측 (2026-06-16)**: 스트림은 **NDJSON**(개행 구분 JSON, `Content-Type: application/stream+json` — SSE 아님)이다.
  - **GA 스트림**: 각 줄 `{answer:<델타조각>, intent, clues}` — `answer`는 **델타**라 이어붙여야 전체 답변이 되고, intent/clues는 매 줄 반복(2KB 답변이 ~1MB로 부풀 수 있음).
  - **run_conversation 스트림**: `{result:{conversation:{id},responses,state}}` 스냅샷 여러 개 + 종료 시 `{type:"final_replaced_message", response_index, content}`(에이전트 최종 답변은 `content`). conversationId는 첫 조각 `result.conversation.id`.
  - 파서(`stream.ts`)는 중괄호-깊이 스캐너라 개행 유무와 무관하게 동작한다(견고).
...
6. 실패 → 에러 코드 해설 표시 (401/`INVALID_TOKEN` "API 키가 유효하지 않습니다 — Settings > General의 REST API 키인지, sdkKey와 혼동하지 않았는지 확인" 등 — §3.3 실측)
...
> ✅ **Gate G1 실 API 검증 완료 (2026-06-16)** — 다수 항목이 실측 확정됨(`plan-results.md` 참조). 확정분은 ✅.
> 주요 추가 발견: 응답 봉투 **비균일**(projects=bare 배열 / apps=`{apps,cursor}` / app상세=bare / hashtags·kb·run=`{result}` / GA=bare `{answer,clues}`),
> 에러 계약은 §3.3 실측표(7000-series는 허구), GA clues 필드는 **snake_case**(`clue_id/page_no/kb_id/faq_id`),
> 앱 타입 분포 campaign/single_action/agent. **5번(Replace/ingestion)은 🔴 파괴적이라 격리 폴더 확보 후 검증 예정.**

1. ✅ **입력 변수 스키마 API 부재** — 확정(앱 상세에 inputs/variables 없음). 단, **빈 inputs 단순 실행 시 에러가 아니라 200 인사말**이 오는 앱도 있음("Expecting value" 미재현) — 불친절 에러는 입력 형식 오류 등 다른 조건에서 발생.
2. ✅ **run_conversation 스트리밍 응답 스키마** — NDJSON(`application/stream+json`), `{result:{conversation:{id},responses,state}}` 스냅샷 + 종료 `{type:"final_replaced_message",content}`. conversationId는 첫 조각 `result.conversation.id` (§3.5).
3. ✅ **run 응답 형태** — 실측 `result.responses[]`(v2, 항목 `{id,type:"llm",message,completed,multipleOption}`). 레거시 `result.choices[]`는 미관측(폴백 유지). 경로 `/webapi/apps/{id}/run`(non-v2).
4. **App Market·Generative Answer는 계약 옵션** — ✅ 이 프로젝트는 GA 활성(200). 모든 고객 기본 제공 아님 — 배포 전 계정 매니저 통해 활성화 확인.
5. **KB 업로드에 overwrite/replace 시맨틱 없음** — `isOverwriteFolderAccess`는 권한 상속 전용. Replace는 반드시 업로드→확인→삭제 조합 (Flow 5). 🔴 격리 폴더 확보 후 검증 예정.
6. ✅ **멀티턴 전제조건** — `OWN-USER-ID` 없으면 응답에서 `threadId` 키 자체가 생략(비활성). 있으면 UUID threadId 반환·재전송 시 유지. 비ASCII는 `base64:` 형식도 서버 수용. `fuQuestion`은 재작성 없으면 `null`.
7. **DraftJS 기본값 함정** — `answerFormat` 미지정 시 DraftJS JSON이 반환됨. SDK는 항상 MARKDOWN 지정 (§3.4). ✅ MARKDOWN→answer 문자열 확인.
