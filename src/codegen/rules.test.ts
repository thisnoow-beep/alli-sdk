...
describe('OWN-USER-ID (§7-2) — 비ASCII는 base64: 변환, ASCII는 그대로', () => {
  it.each(HANGUL_CTX_KEYS)("plan %s — curl 사전 인코딩 리터럴 + JS/Python 헬퍼·원본", (key) => {
    const { curl, browser, node, python } = pick(key);
  ...
  it('plan B — ASCII(EMP12345)는 curl에 변환 없이 그대로', () => {
    const { curl, browser, node, python } = pick('B');
    expect(curl.code).toMatch(/OWN-USER-ID:\s*EMP12345/);
    expect(curl.code).not.toContain('base64:EMP12345');
    for (const a of [browser, node, python]) expect(a.code, a.variant).toContain('EMP12345');
  });
});
...
describe('Content-Type (§7-3) — JSON만 명시, multipart는 boundary 자동', () => {
  it.each(JSON_KEYS)('plan %s(JSON) — 4개 변형 모두 application/json 포함', (key) => {
    for (const a of ARTS[key]) expect(a.code, a.variant).toContain('application/json');
  });
  ...
  it.each(MULTIPART_KEYS)('plan %s(multipart) — application/json 부재 + Content-Type 직접 지정 금지', (key) => {
    ...
    expect(node.code, 'node: FormData 사용').toContain('FormData');
...
describe('stream 소비 (C) — SSE 아님, JSON 조각 누적 파싱', () => {
  it('변형별 스트림 소비 구문 + JSON 추출 헬퍼 동봉', () => {
    ...
    expect(node.code).toContain('for await');
    expect(python.code).toContain('iter_content');
    for (const a of [browser, node, python]) {
      // 청크 누적 → JSON 단위 파싱 헬퍼 (함수명에 json 포함)
...
describe('에러 스켈레톤 (§7-4) — 코드표 해석 주석', () => {
  it.each(KEYS)('plan %s — 4개 변형 모두 7001(잘못된 API 키) 코드표 포함', (key) => {
    for (const a of ARTS[key]) expect(a.code, a.variant).toContain('7001');
  });
});
...
describe('kb-replace (E) — 업로드 → 폴링 → 삭제/롤백', () => {
  it('4개 변형 모두 폴링 경로·종료 상태·구 노드/롤백 삭제·폴링 설정값 포함', () => {
    for (const a of ARTS.E) {
...
describe('멀티턴 (§7-7)', () => {
  it('F(ga-thread-loop) — threadId 보관·재전송 2회 호출 + OWN-USER-ID 전제 주석', () => {
    const { curl, browser, node, python } = pick('F');
    for (const a of ARTS.F) {
  ...
  it('G(conversation-loop) — conversationId 보관 + deep-scan 헬퍼 흔적', () => {
    const { browser, node, python } = pick('G');
    for (const a of ARTS.G) expect(a.code, `${a.variant}: conversationId 부재`).toContain('conversationId');
    for (const a of [browser, node, python]) {
      // 스트리밍 응답 스키마 미문서화(§9-2) → conversationId deep-scan 헬퍼
